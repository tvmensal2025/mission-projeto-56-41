...
import { SessionAssignmentStats } from './SessionAssignmentStats';
import {
  Plus, Edit, Trash2, Send, Clock, Target, 
  BookOpen, Users, CheckCircle, AlertCircle,
  Filter, Search, Calendar, FileText, PlayCircle
} from 'lucide-react';
...
const [formData, setFormData] = useState({
  ...
  const assignSessionToAllUsers = async (sessionId: string) => {
    try {
      // Usar a nova função SQL para atribuir sessão a todos os usuários
      ...
      if (result?.success) {
        toast({
          title: "Sucesso! 🚀",
          description: result.message || "Sessão atribuída com sucesso"
        });
        loadUserSessions();
      } else {
        toast({
  ...
  const assignSessionToSelectedUsers = async (sessionId: string) => {
    try {
      ...
      // Usar a nova função SQL para atribuir sessão aos usuários selecionados
      const { data, error } = await supabase.rpc('assign_session_to_users', {
        session_id_param: sessionId,
        user_ids_param: selectedUserIds,
        admin_user_id: user?.id
      });
      ...
      if (result?.success) {
        toast({
          ...
          description: result.message || `Sessão atribuída para ${result.assignments_created || selectedUserIds.length} usuários`
        });
        setSelectedUserIds([]);
        loadUserSessions();
      } else {
  ...
  const removeSessionAssignment = async (userSessionId: string) => {
    try {
      const { error } = await supabase
        .from('user_sessions')
        .delete()
      ...
      toast({
        title: "Atribuição Removida! 🗑️",
        description: "A sessão foi removida do usuário"
      });
  ...
  const cleanupOrphanedAssignments = async () => {
    try {
      // Delete assignments where user_id doesn't exist in profiles table
      const { data, error } = await supabase
        .from('user_sessions')
        .delete()
        .not('user_id', 'in', `(SELECT user_id FROM profiles WHERE user_id IS NOT NULL)`);

      if (error) throw error;

      toast({
        title: "Limpeza Concluída! 🧹",
        description: "Atribuições órfãs foram removidas da base de dados"
      });
      ...
      toast({
        ...
        description: "Não foi possível remover atribuições órfãs",
        variant: "destructive"
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="sessions" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="sent-sessions">Sessões Enviadas</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="sent-sessions" className="space-y-4">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-lg font-semibold">Sessões Enviadas para Usuários</h3>
              <p className="text-sm text-muted-foreground">
                Gerencie todas as sessões que foram atribuídas aos usuários
              </p>
            </div>
            <div className="flex gap-2">
              <Badge variant="secondary">
                {userSessions.length} atribuições ativas
              </Badge>
              <Button
                variant="outline"
                size="sm"
          ...
          <SessionAssignmentStats userSessions={userSessions} profiles={profiles} />
            ...
            <Card>
              <CardContent className="p-6 text-center">
                <div className="text-muted-foreground">
                  ...
                  <p>Nenhuma sessão foi enviada ainda</p>
                  <p className="text-sm">Use a aba "Sessões" para atribuir sessões aos usuários</p>
                </div>
              </CardContent>
            </Card>
          ) : (
                ...
                <Card key={userSession.id}>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="secondary">
                            {userSession.profiles?.full_name || 'Usuário desconhecido'}
                          </Badge>
                          <Badge variant="outline">
                            {userSession.profiles?.email || 'Sem email'}
                          </Badge>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge 
                          variant={userSession.status === 'completed' ? 'default' : 
                                  userSession.status === 'in_progress' ? 'secondary' : 'outline'}
                        >
                          {userSession.status === 'pending' && <Clock className="w-3 h-3 mr-1" />}
                          {userSession.status === 'in_progress' && <PlayCircle className="w-3 h-3 mr-1" />}
                          {userSession.status === 'completed' && <CheckCircle className="w-3 h-3 mr-1" />}
                          {userSession.status === 'pending' ? 'Pendente' :
                           userSession.status === 'in_progress' ? 'Em Progresso' :
                           userSession.status === 'completed' ? 'Completa' : userSession.status}
                        </Badge>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => removeSessionAssignment(userSession.id)}
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-muted-foreground">Atribuída em:</span>
                        <p>{new Date(userSession.assigned_at).toLocaleString('pt-BR')}</p>
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs">{userSession.progress}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {userSession.sessions.description && (
                      <p className="text-sm text-muted-foreground mt-3">
                        {userSession.sessions.description.substring(0, 100)}...
                      </p>
                    )}
                  </CardContent>
                </Card>
